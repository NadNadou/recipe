import {
  ...
  toNumber,
  trim
} from "./chunk-LLFHS4VU.js";
import "./chunk-LS2IFFW2.js";
import {
  __export
...
var ZoomableContainer = class extends Container {
  ...
  _handleSetWheel() {
    if (this.get("wheelable")) {
      ...
      this._wheelDp = this.events.on("wheel", (event) => {
        ...
        const point5 = this.toLocal(event.point);
        this._handleWheelZoom(wheelEvent.deltaY, point5);
      });
  ...
  _handleWheelZoom(delta, point5) {
    ...
    if (newZoomLevel != zoomLevel) {
      this.zoomToPoint(point5, newZoomLevel);
    }
  }
  /**
  ...
  zoomToPoint(point5, level) {
    ...
    let tx = contents.x();
    let ty = contents.y();
    let xx = cx - (x2 - tx) / zoomLevel * level;
    let yy = cy - (y2 - ty) / zoomLevel * level;
  ...
  _animateTo(x2, y2, scale) {
    const duration = this.get("animationDuration", 0);
    const easing = this.get("animationEasing");
    const contents = this.contents;
    this._txa = contents.animate({ key: "x", to: x2, duration, easing });
    this._tya = contents.animate({ key: "y", to: y2, duration, easing });
  ...
  _handlePinch() {
    ...
    let downPoints = [];
    let movePoints = [];
    each2(this.contents._downPoints, (k2, point5) => {
      downPoints[i] = point5;
      let movePoint = this._movePoints[k2];
      if (movePoint) {
