...
import {
  ...
  export const deleteRecipe = (id) => async dispatch => {
    ...
    // Duplicate a recipe
    export const duplicateRecipe = (id) => async dispatch => {
      try {
        dispatch({ type: DUPLICATE_RECIPE_REQUEST });
        const response = await api.duplicateRecipe(id);
        dispatch({ type: DUPLICATE_RECIPE_SUCCESS, payload: response.data });
      } catch (error) {
        dispatch({
          type: DUPLICATE_RECIPE_FAIL,
          payload: error.response?.data?.message || error.message,
        });
      }
    };
  ...
  export const toggleUpdateRecipe = () => {
    return {
      type: TOGGLE_UPDATE_RECIPE,
    };
  };
